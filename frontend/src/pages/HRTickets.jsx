  ...
  const loadData = async () => {
    try {
      ...
      if (isEmployee && currentUserId) {
        ...
        ticketsData = filter === 'All' ? await api.getTickets() : await api.getTicketsByStatus(filter)
      }
      
      // Only load employees list for admin/HR (to display employee names)
      const employeesData = isAdmin ? await api.getEmployees() : []
    ...
    <div className="space-y-4 md:space-y-6 bg-gray-50 p-4 md:p-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-2 md:gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              className="bg-blue-600 text-white px-4 md:px-6 py-2 md:py-3 rounded-lg md:rounded-xl hover:bg-blue-700 flex items-center gap-2 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 font-semibold text-sm md:text-base"
            >
              <Plus size={20} />
      ...
      <div className="space-y-4">
        ...
        {tickets.map((ticket) => {
          ...
          return (
            <div key={ticket.id} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-4 md:p-6 border-2 border-gray-200 hover:border-blue-300">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-2">
                    <Ticket className="text-blue-600" size={20} />
                    <h3 className="text-base md:text-lg font-semibold text-gray-800">{ticket.subject || 'No Subject'}</h3>
                  ...
                  <p className="text-sm md:text-base text-gray-700">{ticket.description || 'No description'}</p>
                  {ticket.resolution && (
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                <div className="text-xs md:text-sm text-gray-500">
                  ...
                  {ticket.resolvedAt && (
                    <span className="ml-2 md:ml-4">Resolved: {new Date(ticket.resolvedAt).toLocaleDateString()}</span>
                  )}
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-4 md:p-6 w-full max-w-2xl border-2 border-gray-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl md:text-2xl font-bold text-blue-600 flex items-center gap-3">
