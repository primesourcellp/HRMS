...
public class User {
    ...
    // Helper methods for active field (maps to/from employeeStatus)
    public Boolean getActive() {
        if (employeeStatus != null) {
            return "Active".equalsIgnoreCase(employeeStatus);
        }
        if (status != null) {
    ...
    public void setActive(Boolean active) {
        String statusValue = (active != null && active) ? "Active" : "Inactive";
        this.employeeStatus = statusValue;
    ...
    private void ensureRequiredFields() {
        ...
        if (name == null || name.trim().isEmpty()) {
            name = ""; // Set empty string to satisfy NOT NULL constraint
        }
        ...
        // Ensure avatar is set (generate from name initials if not provided)
        if (avatar == null || avatar.trim().isEmpty()) {
            if (name != null && !name.trim().isEmpty()) {
                String[] nameParts = name.trim().split("\\s+");
                if (nameParts.length >= 2) {
                    avatar = (nameParts[0].charAt(0) + "" + nameParts[nameParts.length - 1].charAt(0)).toUpperCase();
                } else if (nameParts.length == 1) {
                    avatar = nameParts[0].substring(0, 1).toUpperCase();
                } else {
